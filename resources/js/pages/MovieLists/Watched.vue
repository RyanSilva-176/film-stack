<template>
  <Head title="Filmes Assistidos" />

  <AppLayout>
    <div class="min-h-screen bg-gray-950">
      <!-- Page Header -->
      <div class="bg-gradient-to-r from-green-600/20 to-green-500/20 border-b border-green-500/30">
        <div class="container mx-auto px-4 py-8 md:py-12">
          <div class="flex items-center gap-4">
            <div class="rounded-full bg-green-600 p-3">
              <font-awesome-icon :icon="['fas', 'eye']" class="h-6 w-6 text-white" />
            </div>
            <div>
              <h1 class="text-2xl md:text-3xl font-bold text-white">Filmes Assistidos</h1>
              <p class="text-gray-300 mt-1">Sua história de filmes assistidos</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <main class="container mx-auto px-4 py-8 md:py-12">
        ...
        <div v-if="loading" class="flex items-center justify-center py-12">
          <div class="text-center space-y-4">
            ...
            <p class="text-gray-400">Carregando filmes assistidos...</p>
          </div>
        </div>
        ...
        <div v-else-if="error" class="flex items-center justify-center py-12">
          <div class="text-center space-y-4">
            ...
            <p class="text-gray-400">Erro ao carregar filmes assistidos</p>
            <button
              @click="loadWatchedMovies"
        ...
        <!-- Empty State -->
        <div v-else-if="!watchedMovies.length" class="flex items-center justify-center py-12">
          <div class="text-center space-y-4 max-w-md">
            <div class="text-gray-500 text-6xl">🎬</div>
            <h2 class="text-xl font-semibold text-white">Nenhum filme assistido ainda</h2>
            <p class="text-gray-400">
              Marque os filmes que você assistiu para manter um histórico!
            </p>
            <router-link
              to="/dashboard"
        ...
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
          <MovieCard
            v-for="movieItem in watchedMovies"
            :key="movieItem.id"
...
import { faEye, faCompass, faChevronLeft, faChevronRight } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/vue-fontawesome';
import type { Movie } from '@/types/movies';

library.add(faEye, faCompass, faChevronLeft, faChevronRight);
...
const watchedMovies = computed(() => userListsStore.currentListMovies);
...
const loadWatchedMovies = async () => {
  ...
  if (watchedList) {
    await userListsStore.fetchListMovies(watchedList.id);
  }
};
...
const loadPage = async (page: number) => {
  const watchedList = userListsStore.getWatchedList;
  if (watchedList) {
    await userListsStore.fetchListMovies(watchedList.id, page);
  }
};

onMounted(async () => {
  // Load user lists first
  await userListsStore.fetchUserLists();
  
  // Then load watched movies
